...
// Only payment_date, payment_mode, reference_number, notes, crm_receipt_no and
// legal_entity_id can be patched here — amount, allocations and status affect invoice
// balances and the auto-journal, so changing those still requires editing the payment in the
// app itself. See src/lib/receipts-api.ts.
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const result = await updateReceiptMeta(ctx.orgId, params.id, body, { apiKeyId: ctx.apiKeyId });
